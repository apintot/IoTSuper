#include <WiFi.h>
#include <WiFiClientSecure.h>
#include <PubSubClient.h>
#include "HX711.h"
#include <EEPROM.h>

//*****Constantes******//

const char* routerName = "MOVISTAR-Redalberto";
const char* routerPassword = "AjhkXV6hrFqb7yxqkxTU";

const char* mqttServer = "iotsuper.duckdns.org";
const int mqttPort = 8883;

const char* usuarioMqtt = "iotsuper";
const char* contrasenaMqtt = "iotsupermqtt";

String chipIDStr;
String topic;

WiFiClientSecure espClient;
PubSubClient cliente(espClient);

const int HX711_ot = 4;
const int HX711_sck  = 5;

HX711 celda;

const int   calVal_eepromAdress = 0;
float       calibrationValue;

unsigned long ultimaPublicacion = 0;

const char* cert = R"(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)";

//************************//

/**********MQTT CONECTANDO*****************/

void conectandoMQTT()
{
  while(!cliente.connected())
  {
    Serial.print("Conectando al servidor mqtt...");

    if(cliente.connect(chipIDStr.c_str(), usuarioMqtt, contrasenaMqtt))
    {
      Serial.println("Conectado!");
    }
    else
    {
      Serial.printf("Error: %d\n", cliente.state());
      delay(2000);
    }
  }
}

/******************************************/

void setup() 
{
Serial.begin(57600);
  Serial.println("Starting...");

  celda.begin(HX711_ot, HX711_sck);		// inicializa objeto con los pines a utilizar

  celda.set_scale(413.74f);	// establece el factor de escala obtenido del primer programa
  celda.tare();	

  // Luego WiFi y MQTT
  uint64_t chipID = ESP.getEfuseMac();
  chipIDStr = String((uint32_t)(chipID >> 32), HEX) + String((uint32_t)chipID, HEX);
  topic = "IoTSuper/STK/" + chipIDStr;

  WiFi.begin(routerName, routerPassword);
  Serial.print("Conectando WiFi");
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
  Serial.println(" conectado!");

  configTime(0, 0, "pool.ntp.org", "time.nist.gov");
  Serial.print("Sincronizando hora");
  struct tm timeinfo;
  while (!getLocalTime(&timeinfo)) {
    Serial.print(".");
    delay(500);
  }
  Serial.println(" lista!");

  espClient.setCACert(cert);
  cliente.setServer(mqttServer, mqttPort);
  cliente.setBufferSize(512);
  conectandoMQTT();

  Serial.println("Startup completo");
}

void loop() 
{
  if (!cliente.connected()) conectandoMQTT();

  cliente.loop();

  float peso = celda.get_units(10);
  celda.power_down();
  delay(3000);
  celda.power_up();	

  char buffer[10];
  dtostrf(peso, 6, 2, buffer);

  if (cliente.publish(topic.c_str(), buffer)) 
  {
    Serial.printf("Publicado: %s gr\n", buffer);
  } 
  else 
  {
    Serial.println("Error publicando");
  }
}
